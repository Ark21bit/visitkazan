<template>
    <div>
<<<<<<< HEAD
        <h2 class="[&>*]:m-1 font-Nekst text-6.5 lg:text-4xl text-fblack">Наши программы</h2>
        <div class="mt-6 lg:mt-12.5 grid grid-cols-1 lg:grid-cols-2 justify-between gap-7.5">
            <div class="grid gap-x-6 grid-cols-1 sm:lt-lg:grid-cols-2">
                <div class="flex w-full gap-6 sm:lt-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-1.4 text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="lt-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-455px">
                            <div>Классические программы</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-.25"></div>
                                <div class="group-hover/program:w-65px transition-width duration-500 ease-linear w-25px h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-.25"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-134px sm:w-181px shrink-0">
                        <img src="/img.png" class="object-cover rounded-2.5 w-full h-85px sm:h-113px" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:lt-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-1.4 text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="lt-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-455px">
                            <div>Туры для школьников</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-.25"></div>
                                <div class="group-hover/program:w-65px transition-width duration-500 ease-linear w-25px h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-.25"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-134px sm:w-181px shrink-0">
                        <img src="/img.png" class="object-cover rounded-2.5 w-full h-85px sm:h-113px" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:lt-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-1.4 text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="lt-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-455px">
                            <div>Туры на выходные</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-.25"></div>
                                <div class="group-hover/program:w-65px transition-width duration-500 ease-linear w-25px h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-.25"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-134px sm:w-181px shrink-0">
                        <img src="/img.png" class="object-cover rounded-2.5 w-full h-85px sm:h-113px" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:lt-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-1.4 text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="lt-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-455px">
                            <div>Эконом туры</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-.25"></div>
                                <div class="group-hover/program:w-65px transition-width duration-500 ease-linear w-25px h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-.25"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-134px sm:w-181px shrink-0">
                        <img src="/img.png" class="object-cover rounded-2.5 w-full h-85px sm:h-113px" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:lt-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-1.4 text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="lt-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-455px">
                            <div>VIP программы</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-.25"></div>
                                <div class="group-hover/program:w-65px transition-width duration-500 ease-linear w-25px h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-.25"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-134px sm:w-181px shrink-0">
                        <img src="/img.png" class="object-cover rounded-2.5 w-full h-85px sm:h-113px" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:lt-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-1.4 text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="lt-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-455px">
                            <div>1001 удовольствие</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-.25"></div>
                                <div class="group-hover/program:w-65px transition-width duration-500 ease-linear w-25px h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-.25"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-134px sm:w-181px shrink-0">
                        <img src="/img.png" class="object-cover rounded-2.5 w-full h-85px sm:h-113px" alt="">
                    </div>
                </div>
            </div>
            <div class="relative lt-lg:hidden">
                <img src="/img.png" class="rounded-3.75 w-full h-full absolute object-cover" alt="">
=======
        <h2 class="font-Nekst text-[26px] lg:text-4xl text-fblack">Наши программы</h2>
        <div class="mt-6 lg:mt-12.5 grid grid-cols-1 lg:grid-cols-2 justify-between gap-7.5">
            <div class="grid gap-x-6 grid-cols-1 sm:max-lg:grid-cols-2">
                <div class="flex w-full gap-6 sm:max-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-[1.4] text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="max-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-[455px]">
                            <div>Классические программы</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                                <div class="group-hover/program:w-[65px] transition-[width] duration-500 ease-linear w-[25px] h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-[134px] sm:w-[181px] shrink-0">
                        <img src="/img.png" class="object-cover rounded-[10px] w-full h-[85px] sm:h-[113px]" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:max-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-[1.4] text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="max-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-[455px]">
                            <div>Туры для школьников</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                                <div class="group-hover/program:w-[65px] transition-[width] duration-500 ease-linear w-[25px] h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-[134px] sm:w-[181px] shrink-0">
                        <img src="/img.png" class="object-cover rounded-[10px] w-full h-[85px] sm:h-[113px]" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:max-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-[1.4] text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="max-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-[455px]">
                            <div>Туры на выходные</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                                <div class="group-hover/program:w-[65px] transition-[width] duration-500 ease-linear w-[25px] h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-[134px] sm:w-[181px] shrink-0">
                        <img src="/img.png" class="object-cover rounded-[10px] w-full h-[85px] sm:h-[113px]" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:max-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-[1.4] text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="max-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-[455px]">
                            <div>Эконом туры</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                                <div class="group-hover/program:w-[65px] transition-[width] duration-500 ease-linear w-[25px] h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-[134px] sm:w-[181px] shrink-0">
                        <img src="/img.png" class="object-cover rounded-[10px] w-full h-[85px] sm:h-[113px]" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:max-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-[1.4] text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="max-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-[455px]">
                            <div>VIP программы</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                                <div class="group-hover/program:w-[65px] transition-[width] duration-500 ease-linear w-[25px] h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-[134px] sm:w-[181px] shrink-0">
                        <img src="/img.png" class="object-cover rounded-[10px] w-full h-[85px] sm:h-[113px]" alt="">
                    </div>
                </div>
                <div class="flex w-full gap-6 sm:max-lg:[&:nth-child(2)]:border-t first:border-t py-5 border-b border-fline">
                    <div class="grow flex justify-between gap-2 group/program text-lg lg:text-xl font-medium leading-[1.4] text-fblack hover:text-fblue transition-colors duration-500 ease-linear">
                        <span class="max-lg:hidden">01</span>
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between grow max-w-[455px]">
                            <div>1001 удовольствие</div>
                            <div class="w-fit relative flex items-center">
                                <div class="absolute h-0.5 w-2 bg-current right-0 rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                                <div class="group-hover/program:w-[65px] transition-[width] duration-500 ease-linear w-[25px] h-0.5 bg-current rounded-full"></div>
                                <div class="absolute h-0.5 w-2 bg-current right-0 -rotate-45 origin-right rounded-full -translate-x-[1px]"></div>
                            </div>
                        </div>
                    </div>
                    <div class="lg:hidden w-[134px] sm:w-[181px] shrink-0">
                        <img src="/img.png" class="object-cover rounded-[10px] w-full h-[85px] sm:h-[113px]" alt="">
                    </div>
                </div>
            </div>
            <div class="relative max-lg:hidden">
                <img src="/img.png" class="rounded-[15px] w-full h-full absolute object-cover" alt="">
>>>>>>> deef4cf (fix)
            </div>
        </div>
    </div>
</template>

<script setup>

</script>