    ...
    <Teleport to="#teleported">
        <Transition enter-from-class="opacity-0" leave-to-class="opacity-0" enter-active-class="transition-opacity duration-500 easy-linear" leave-active-class="transition-opacity duration-500 easy-linear">
            <template v-if="isShow">
                <div :class="modalOverlayClass">
                    <div :class="modalWrapperClass">
                        <!-- Modal content -->
                        <div v-bind="$attrs" :class="modalClass" v-on-click-outside="closeModal">
...
const modalOverlayClass = computed(() => {
    return [
        checkDefaultClass(props.overlayClass, 'bg-black/50 fixed z-40 overflow-y-auto top-0 left-0 w-full h-screen')
    ]
})
...
const modalClass = computed(() => {
    return [
        props.noDefaultAttrsClass || 'relative bg-white rounded-lg p-5 lg:p-7.5 w-[calc(100%-var(--width-content-margin)*2)]',
...
const modalWrapperClass = computed(() => {
    return [
        checkDefaultClass(props.wrapperClass, 'overflow-x-hidden z-50 min-h-screen w-full py-6 justify-center items-center flex'),
    ]
})
