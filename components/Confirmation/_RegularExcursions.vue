    ...
    <div class="flex mx-auto w-full flex-col gap-7.5 mt-7.5 rounded-3.75 shadow-[0_4px_23px_0_#0001] px-4 py-5 lg:p-7.5 max-w-854px">
        <div class="flex flex-col gap-4 lg:gap-3">
            ...
            <ConfirmationBaseBlock>
                ...
                <template #label>{{ orderInfo?.time?.label }}</template>
            </ConfirmationBaseBlock>
            <ConfirmationBaseBlock>
                <template #title>{{ generalConfig?.static_info?.global_words?.starting_point }}</template>
                <template #label>{{ getTitlePlacesStart(orderInfo?.place_start_id) }}</template>
            </ConfirmationBaseBlock>
            <ConfirmationBaseBlock>
            ...
            <ConfirmationBaseBlock>
                ...
                <template #label>{{ orderInfo?.coupon?.id ? `${generalConfig?.static_info?.global_words?.sale} ${orderInfo?.coupon?.sale_present}%` : '-' }}</template>
            </ConfirmationBaseBlock>
            <ConfirmationBaseBlock>
        ...
        <div class="flex flex-col gap-5">
            ...
            <div class="border-t pt-4 lg:pt-0 border-fline2 lg:border-none flex flex-col gap-3">
                <ConfirmationBaseBlock v-for="item in filterTickets">
                    <template #title>{{ getTitleTypeTicket(item?.id) }} ({{ item?.count }})</template>
...
const storeOrderInfo = computed(() => {
    ...
    if (orderInfo.value?.additional?.length > 0) object.additional = filterMap(orderInfo.value.additional,
        (a) => a.count > 0,
        (a) => { return { id: a.id, count: a.count } })
