<template>
    <FormKit v-model="forms" @submit="setOrder" type="form" form-class="flex flex-col gap-y-7.5 relative" :actions="false">
        ...
        <div class="flex flex-col gap-5">
            <h4 class="font-Nekst col-span-full text-base leading-1.2 text-fblack">{{ generalConfig?.static_info?.global_words?.count_people }}</h4>
            <FormKit name="type_price_id" validation="required" type="selectC" :options="typePrices" :label="generalConfig?.static_info?.global_words?.count_people"></FormKit>
        </div>
        <div class="flex flex-col gap-5">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <FormKit name="client_telephone" validation="required:trim" type="text" v-maska data-maska="###" :label="generalConfig?.static_info?.global_words?.number_telephone"></FormKit>
...
const hotels = computedWithControl(() => forms.value?.type_price_id, () =>
    filterMap(props.product?.info_prices?.data,
        (a) => a.type_price_id == forms.value?.type_price_id,
        (a) => { return { value: { value: a.hotel_id, label: a?.hotel_info?.title }, label: a?.hotel_info?.title } }
    )
)
