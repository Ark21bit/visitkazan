<template>
    <div class="flex h-full flex-col rounded-2.5 lg:rounded-3.75 bg-white overflow-hidden shadow-[0_4px_23px_0_#0001] ring-2 ring-transparent hover:ring-fblue transition-shadow duration-500 ease-linear">
        <div class="h-204px sm:h-237px lg:h-230px [&>img]:(w-full h-full object-cover)" v-html="img">
        </div>
        <div class="flex flex-col flex-cal gap-4 p-5 border-b border-fline grow">
            ...
            <Rating size="20" :rating="rating" class="text-fblue"></Rating>
        </div>
        <div class="p-5">
            <Button class="w-full" :to="slug" :tag="CustomLink">{{ generalConfig?.static_info?.global_words?.more }}</Button>
        </div>
    </div>
</template>
<script setup>
